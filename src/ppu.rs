...
impl PPU {
    pub fn new(cart: Rc<RefCell<Cartridge>>) -> Self {
        Self {
            ...
            spr_name_table: [Rc::new(RefCell::new(olc::Sprite::with_dims(256, 240))), Rc::new(RefCell::new(olc::Sprite::with_dims(256, 240)))],
            spr_pattern_table: [Rc::new(RefCell::new(olc::Sprite::with_dims(128, 128))), Rc::new(RefCell::new(olc::Sprite::with_dims(128, 128)))],
    ...
    fn increment_scroll_y(&mut self) {
        ...
        if fine_y < 7 {
            ...
            if let 29 = coarse_y { // yes, reset coarse y
    ...
    fn can_render(&self) -> bool {
        if self.mask & (MASK_RENDER_BACKGROUND | MASK_RENDER_SPRITES) > 0 { return true; }
        false
    }
    ...
    pub fn clock(&mut self) {
        match self.scanline {
            -1..=239 => {
                match (self.scanline, self.cycle) {
                    ...
                    (_, 2..=257 | 321..=337) => {
                        ...
                        match (self.cycle - 1) % 8 {
                            ...
                            2 => {
                                ...
                                if ((self.vram_addr & REG_COARSE_Y) >> 5) & 0x02 > 0 { self.bg_next_tile_attrib >>= 4 }
                                if ((self.vram_addr & REG_COARSE_X)) & 0x02 > 0 { self.bg_next_tile_attrib >>= 2 }
                    ...
                    (_, 338 | 340) => { self.bg_next_tile_id = self.ppu_read(0x2000 | (self.vram_addr & 0x0FFF))},
        ...
        if self.mask & MASK_RENDER_BACKGROUND > 0{
            ...
            bg_pixel = (p1_pixel << 8) | p0_pixel;

            let bg_pal0: u8 = (self.bg_shifter_attrib_lo & bit_mux) as u8;
            let bg_pal1: u8 = (self.bg_shifter_attrib_hi & bit_mux) as u8;
            bg_palette = (bg_pal1 << 8) | bg_pal0;
        }

        self.spr_screen.borrow_mut().set_pixel((self.cycle as i32) -1, self.scanline as i32, self.get_color_from_palette_ram(bg_palette, bg_pixel));
    ...
    pub fn cpu_read(&mut self, addr: u16, read_only: bool) -> u8 {
        match (addr, read_only) {
            ...
            (0x0007, false) => {
                ...
                self.vram_addr += if self.ctrl & CTRL_INCREMENT_MODE > 0 { 32 } else { 1 };
                return data;
            } // PPU Data
            _ => {
                return 0;
    ...
    pub fn cpu_write(&mut self, addr: u16, data: u8) {
        match addr {
            0x0000 => {
                ...
                self.tram_addr |= ((self.ctrl & (CTRL_NAMETABLE_X | CTRL_NAMETABLE_Y)) as u16) << 10;
            } // Control
    ...
    pub fn ppu_read(&self, addr: u16) -> u8 {
        ...
        if let Ok(x) = self.cart.borrow().ppu_read(local_addr) {
            ...
            return self.pattern_table[((local_addr & 0x1000) >> 12) as usize][(local_addr & 0x0FFF) as usize]
            ...
            if let 0x0010 | 0x0014 | 0x0018 | 0x001C = local_addr { local_addr &= !0x0010 }            
    ...
    pub fn ppu_write(&mut self, addr: u16, data: u8) {
        ...
        if let Ok(_) = self.cart.borrow_mut().ppu_write(addr, data) {}
            ...
            self.pattern_table[((local_addr & 0x1000) >> 12) as usize][(local_addr & 0x0FFF) as usize] = data;
    ...
    fn grayscale(&self) -> u8 {
        if self.mask & MASK_GRAYSCALE > 0 { 0x30 } else { 0x3F }
    }
    ...
    pub fn get_pattern_table(&self, index: usize, palette_id: u8) -> Rc<RefCell<olc::Sprite>> {
        if !(0..=1).contains(&index) { return Rc::new(RefCell::new(olc::Sprite::with_dims(128, 128))); }
    ...
    pub fn get_color_from_palette_ram(&self, palette: u8, pixel: u8) -> olc::Pixel {
        let idx = self.ppu_read(0x3F00 + ((palette << 2) + pixel) as u16);
    ...
    fn init_pal_screen() -> [olc::Pixel; 0x40] {
        let mut pal_screen = [olc::Pixel { r: 0, g: 0, b: 0, a: 0 }; 0x40];
        pal_screen[0x00] = olc::Pixel { r: 84, g: 84, b: 84, a: 0 };
        pal_screen[0x01] = olc::Pixel { r: 0, g: 30, b: 116, a: 0 };
        pal_screen[0x02] = olc::Pixel { r: 8, g: 16, b: 144, a: 0 };
        pal_screen[0x03] = olc::Pixel { r: 48, g: 0, b: 136, a: 0 };
        pal_screen[0x04] = olc::Pixel { r: 68, g: 0, b: 100, a: 0 };
        pal_screen[0x05] = olc::Pixel { r: 92, g: 0, b: 48, a: 0 };
        pal_screen[0x06] = olc::Pixel { r: 84, g: 4, b: 0, a: 0 };
        pal_screen[0x07] = olc::Pixel { r: 60, g: 24, b: 0, a: 0 };
        pal_screen[0x08] = olc::Pixel { r: 32, g: 42, b: 0, a: 0 };
        pal_screen[0x09] = olc::Pixel { r: 8, g: 58, b: 0, a: 0 };
        pal_screen[0x0A] = olc::Pixel { r: 0, g: 64, b: 0, a: 0 };
        pal_screen[0x0B] = olc::Pixel { r: 0, g: 60, b: 0, a: 0 };
        pal_screen[0x0C] = olc::Pixel { r: 0, g: 50, b: 60, a: 0 };
        pal_screen[0x0D] = olc::Pixel { r: 0, g: 0, b: 0, a: 0 };
        pal_screen[0x0E] = olc::Pixel { r: 0, g: 0, b: 0, a: 0 };
        pal_screen[0x0F] = olc::Pixel { r: 0, g: 0, b: 0, a: 0 };
    
        pal_screen[0x10] = olc::Pixel { r: 152, g: 150, b: 152, a: 0 };
        pal_screen[0x11] = olc::Pixel { r: 8, g: 76, b: 196, a: 0 };
        pal_screen[0x12] = olc::Pixel { r: 48, g: 50, b: 236, a: 0 };
        pal_screen[0x13] = olc::Pixel { r: 92, g: 30, b: 228, a: 0 };
        pal_screen[0x14] = olc::Pixel { r: 136, g: 20, b: 176, a: 0 };
        pal_screen[0x15] = olc::Pixel { r: 160, g: 20, b: 100, a: 0 };
        pal_screen[0x16] = olc::Pixel { r: 152, g: 34, b: 32, a: 0 };
        pal_screen[0x17] = olc::Pixel { r: 120, g: 60, b: 0, a: 0 };
        pal_screen[0x18] = olc::Pixel { r: 84, g: 90, b: 0, a: 0 };
        pal_screen[0x19] = olc::Pixel { r: 40, g: 114, b: 0, a: 0 };
        pal_screen[0x1A] = olc::Pixel { r: 8, g: 124, b: 0, a: 0 };
        pal_screen[0x1B] = olc::Pixel { r: 0, g: 118, b: 40, a: 0 };
        pal_screen[0x1C] = olc::Pixel { r: 0, g: 102, b: 120, a: 0 };
        pal_screen[0x1D] = olc::Pixel { r: 0, g: 0, b: 0, a: 0 };
        pal_screen[0x1E] = olc::Pixel { r: 0, g: 0, b: 0, a: 0 };
        pal_screen[0x1F] = olc::Pixel { r: 0, g: 0, b: 0, a: 0 };
    
        pal_screen[0x20] = olc::Pixel { r: 236, g: 238, b: 236, a: 0 };
        pal_screen[0x21] = olc::Pixel { r: 76, g: 154, b: 236, a: 0 };
        pal_screen[0x22] = olc::Pixel { r: 120, g: 124, b: 236, a: 0 };
        pal_screen[0x23] = olc::Pixel { r: 176, g: 98, b: 236, a: 0 };
        pal_screen[0x24] = olc::Pixel { r: 228, g: 84, b: 236, a: 0 };
        pal_screen[0x25] = olc::Pixel { r: 236, g: 88, b: 180, a: 0 };
        pal_screen[0x26] = olc::Pixel { r: 236, g: 106, b: 100, a: 0 };
        pal_screen[0x27] = olc::Pixel { r: 212, g: 136, b: 32, a: 0 };
        pal_screen[0x28] = olc::Pixel { r: 160, g: 170, b: 0, a: 0 };
        pal_screen[0x29] = olc::Pixel { r: 116, g: 196, b: 0, a: 0 };
        pal_screen[0x2A] = olc::Pixel { r: 76, g: 208, b: 32, a: 0 };
        pal_screen[0x2B] = olc::Pixel { r: 56, g: 204, b: 108, a: 0 };
        pal_screen[0x2C] = olc::Pixel { r: 56, g: 180, b: 204, a: 0 };
        pal_screen[0x2D] = olc::Pixel { r: 60, g: 60, b: 60, a: 0 };
        pal_screen[0x2E] = olc::Pixel { r: 0, g: 0, b: 0, a: 0 };
        pal_screen[0x2F] = olc::Pixel { r: 0, g: 0, b: 0, a: 0 };
    
        pal_screen[0x30] = olc::Pixel { r: 236, g: 238, b: 236, a: 0 };
        pal_screen[0x31] = olc::Pixel { r: 168, g: 204, b: 236, a: 0 };
        pal_screen[0x32] = olc::Pixel { r: 188, g: 188, b: 236, a: 0 };
        pal_screen[0x33] = olc::Pixel { r: 212, g: 178, b: 236, a: 0 };
        pal_screen[0x34] = olc::Pixel { r: 236, g: 174, b: 236, a: 0 };
        pal_screen[0x35] = olc::Pixel { r: 236, g: 174, b: 212, a: 0 };
        pal_screen[0x36] = olc::Pixel { r: 236, g: 180, b: 176, a: 0 };
        pal_screen[0x37] = olc::Pixel { r: 228, g: 196, b: 144, a: 0 };
        pal_screen[0x38] = olc::Pixel { r: 204, g: 210, b: 120, a: 0 };
        pal_screen[0x39] = olc::Pixel { r: 180, g: 222, b: 120, a: 0 };
        pal_screen[0x3A] = olc::Pixel { r: 168, g: 226, b: 144, a: 0 };
        pal_screen[0x3B] = olc::Pixel { r: 152, g: 226, b: 180, a: 0 };
        pal_screen[0x3C] = olc::Pixel { r: 160, g: 214, b: 228, a: 0 };
        pal_screen[0x3D] = olc::Pixel { r: 160, g: 162, b: 160, a: 0 };
        pal_screen[0x3E] = olc::Pixel { r: 0, g: 0, b: 0, a: 0 };
        pal_screen[0x3F] = olc::Pixel { r: 0, g: 0, b: 0, a: 0 };
        pal_screen
    }
}
...
// Status
const STATUS_UNUSED_MASK:         u8 = 0b0001_1111;
const STATUS_SPRT_OVERFLOW_MASK:  u8 = 0b0010_0000;
const STATUS_SPRT_HIT_ZERO_MASK:  u8 = 0b0100_0000;
const STATUS_VERTICAL_BLANK_MASK: u8 = 0b1000_0000;

// Mask 
const MASK_GRAYSCALE:              u8 = 0b0000_0001;
const MASK_RENDER_BACKGROUND_LEFT: u8 = 0b0000_0010;
const MASK_RENDER_SPRITES_LEFT:    u8 = 0b0000_0100;
const MASK_RENDER_BACKGROUND:      u8 = 0b0000_1000;
const MASK_RENDER_SPRITES:         u8 = 0b0001_0000;
const MASK_ENHANCE_RED:            u8 = 0b0010_0000;
const MASK_ENHANCE_GREEN:          u8 = 0b0100_0000;
const MASK_ENHANCE_BLUE:           u8 = 0b1000_0000;

// CTRL
const CTRL_NAMETABLE_X:        u8 = 0b0000_0001;
const CTRL_NAMETABLE_Y:        u8 = 0b0000_0010;
const CTRL_INCREMENT_MODE:     u8 = 0b0000_0100;
const CTRL_PATTERN_SPRITE:     u8 = 0b0000_1000;
const CTRL_PATTERN_BACKGROUND: u8 = 0b0001_0000;
const CTRL_SPRITE_SIZE:        u8 = 0b0010_0000;
const CTRL_SLAVE_MODE:         u8 = 0b0100_0000;
const CTRL_ENABLE_NMI:         u8 = 0b1000_0000;


// PPU Register
const REG_COARSE_X:    u16 = 0b0000_0000_0001_1111;
const REG_COARSE_Y:    u16 = 0b0000_0011_1110_0000;
const REG_NAMETABLE_X: u16 = 0b0000_0100_0000_0000;
const REG_NAMETABLE_Y: u16 = 0b0000_1000_0000_0000;
const REG_FINE_Y:      u16 = 0b0111_0000_0000_0000;
const REG_UNUSED :     u16 = 0b1000_0000_0000_0000;
